...
read_relatedness_matrix <- function(file) {
  raw <- utils::read.delim(file, header = TRUE, stringsAsFactors = FALSE,
                     check.names = FALSE)
...
.best_group_combinations <- function(candidates, n_groups, max_combo_avg_rel,
  ...
  n_combos<- choose(length(candidates), n_groups)
  if ( n_combos < 200000000){
    idx_combos <- utils::combn(length(candidates), n_groups, simplify = FALSE)
  }else {
    stop(
      sprintf(
        "SWINGER would need to generate %.0f possible group combinations. This exceeds the current memory limits of R and available RAM. Please increase filtering stringency or reduce the number of groups requested.",
        n_combos)
    )}
...
#' print_matrix_groups(results, kinship_df)
#' }
...
#' @return The combination of breeding groups with everage relatednes/kinship per group and combination from swingeR resutls.
#' @export
#'
#' @examples
#' \dontrun{
#' BreedGroups <- print_breeding_groups(outputSwingeR)
#' }
...
# ---- 8. KR-matrix-print helper ------------------------------------------------

#' Print relatedness/kinship matrix of the combination of groups selected by swingeR().
#'
#' @param outputswinger Output of \code{\link{swingeR}}.
#' @param kr_df original relatedness/kinship data frame used as input for \code{\link{swingeR}}.
#' @return A set of pairwise kinship/relatedness matrix for each group in the swingeR resutls.
#' @export
#'
#' @examples
#' \dontrun{
#' myBredGMatrix <- print_matrix_groups(outputswinger, kinship_df)
#' }
print_matrix_groups<-function(outputswinger, kr_df) {
  
  lapply(outputswinger[[1]]$groups, function(g) {
    ids <- g$ids
    mat <- matrix(
      NA_real_,
      nrow = length(ids),
